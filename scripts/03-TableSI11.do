...
* ==================================================================
*
* Description: Tests robustness by restricting sample to neighborhoods
*   in low-migration counties (bottom 50% of migration rate distribution).
*
* Columns:
*   (1) OLS: Distance + metro FE
*   (2) OLS: + Geography + state FE
*   (3) IV: Distance + metro FE
*   (4) IV: + Geography + state FE
*
* Outcomes:
...
*
* Outputs:
*   - table_low_migration_[outcome].tex
*
* ==================================================================
...
* ------------------------------------------------------------------
* 1.2 Define low-migration sample
* ------------------------------------------------------------------

* Create quartiles of migration rate
xtile migration_rate2000_quant = migration_rate2000, n(4)

* Bottom 50% indicator
gen migration_rate2000_bottom50 = (migration_rate2000_quant <= 2)

* ------------------------------------------------------------------
* 1.3 Create instrument
* ------------------------------------------------------------------
...
* Migration sample restriction
global migration migration_rate2000_bottom50
...
foreach yvar in sg_log_interaction_norm sg_social_homedwell outcome_mob_GHG_all_trips {
    ...
    * ------------------------------------------------------------------
    * 3.1 OLS Regressions (restricted to low-migration counties)
    * ------------------------------------------------------------------

    * Column 1: Distance + metro FE
    reghdfe $depvar ${gdi} ${dist} if ${migration} == 1 ///
        [aw=census2000blkgp_dem_total_pop], absorb(controlsgeo_metro)
    if e(N_full) == . {
        estadd scalar N_full = e(N)
    }
    estimates store e1

    * Column 2: + Geography + state FE
    reghdfe $depvar ${gdi} ${dist} ${geo_controls} if ${migration} == 1 ///
        [aw=census2000blkgp_dem_total_pop], absorb(controlsgeo_metro)
    if e(N_full) == . {
        estadd scalar N_full = e(N)
    }
    ...
    * ------------------------------------------------------------------
    * 3.2 IV Regressions (restricted to low-migration counties)
    * ------------------------------------------------------------------

    * Column 3: Distance + metro FE
    ivreghdfe $depvar (${gdi} = ${instrument_list}) ${dist} if ${migration} == 1 ///
        [aw=census2000blkgp_dem_total_pop], absorb(controlsgeo_metro)
    if e(N_full) == . {
        estadd scalar N_full = e(N)
    }
    ...
    * ------------------------------------------------------------------
    ...
    estout e1 e2 e3 e4 using "${tables}/table_low_migration_${depvar}.tex", ///
        style(tex) ///
        varlabels(${gdi} "`ltit'") ///
    ...
    di "Table saved: table_low_migration_${depvar}.tex"
...
di _n "All low-migration robustness tables saved to: ${tables}"
